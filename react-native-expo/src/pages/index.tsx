    ...
    <View style={styles.page}>
      <ScrollView contentContainerStyle={[styles.scroll, banners.length > 0 && styles.withBanner]} showsVerticalScrollIndicator={false}>
        {banners.length > 0 && (
          <View style={styles.bannerWrap}>
            <ScrollView
              ...
              onMomentumScrollEnd={event => setBannerIndex(Math.round(event.nativeEvent.contentOffset.x / bannerWidth))}
            >
              {banners.map(item => (
                <Pressable key={String(item.id)} onPress={() => openItem(item)}>
                  <Image source={{ uri: scaleCosImage(item.image, 750) }} style={[styles.banner, { width: bannerWidth }]} />
                </Pressable>
              ))}
            </ScrollView>
            <View style={styles.dots}>
              {banners.map((item, index) => (
                <View key={String(item.id)} style={[styles.dot, index === bannerIndex && styles.dotActive]} />
              ))}
            </View>
          </View>
        )}
        ...
        <View style={styles.featureArea}>
          <View style={styles.featureCard}>
            {HOME_FEATURES.map(feature => (
              <Pressable key={feature.name} onPress={() => router.push(feature.href as never)} style={styles.feature}>
            ...
            <View style={styles.list}>
              {articles.map(article => (
                <Pressable
                  ...
                  onPress={() => openItem(article, `/article/detail?id=${article.id}`)}
                  style={styles.article}
                >
                  <Image source={{ uri: scaleCosImage(article.thumb, 120) }} style={styles.articleImage} />
                  <View style={styles.articleContent}>
                    <Text numberOfLines={1} style={styles.articleTitle}>{String(article.title ?? '')}</Text>
                    <Text numberOfLines={2} style={styles.articleDesc}>{String(article.note ?? '')}</Text>
                  </View>
                </Pressable>
              ))}
            </View>
        ...
        <View style={styles.version}>
          <Text style={styles.versionText}>你好，{userName}！</Text>
          <Text style={styles.versionText}>{APP_NAME}@{APP_VERSION} {isMock ? '[mock]' : ''}</Text>
        </View>
      </ScrollView>
