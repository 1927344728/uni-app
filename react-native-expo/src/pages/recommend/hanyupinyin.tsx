...
const types: { value: Type; name: string }[] = [{ value: 'all', name: '全部（63）' }, { value: 'shengmu', name: '声母（23）' }, { value: 'yunmu', name: '韵母（24）' }, { value: 'whole', name: '整体认读（16）' }];
...
export default function HanyuPinyinScreen() {
  ...
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.scroll}>
    <View style={styles.header}><Text style={styles.heading}>汉语拼音发音学习</Text><Text style={styles.description}>点击任意拼音即可播放发音</Text></View>
    <View style={styles.card}><View style={styles.categories}>{types.map(option => <Pressable key={option.value} onPress={() => { stop(); setType(option.value); setSearch(''); const first = option.value === 'all' ? entries[0] : entries.find(item => item.type === option.value); setSelected(first?.value ?? null); }} style={[styles.category, type === option.value && styles.categoryActive]}><Text style={[styles.categoryText, type === option.value && styles.white]}>{option.name}</Text></Pressable>)}</View>
      <View style={styles.search}><TextInput value={search} onChangeText={setSearch} onSubmitEditing={() => { const item = entries.find(entry => entry.value === search.trim()); setSelected(item?.value ?? null); }} placeholder="搜索拼音..." style={styles.input} /><Pressable style={styles.searchButton} onPress={() => { const item = entries.find(entry => entry.value === search.trim()); setSelected(item?.value ?? null); }}><Text style={styles.white}>搜索</Text></Pressable></View>
      <View style={styles.controls}><Pressable style={[styles.playButton, !selected && styles.disabled]} onPress={() => selected ? speak(entries.find(item => item.value === selected)?.name ?? selected) : Alert.alert('提示', '请选择拼音')}><Text style={styles.white}>播放选中</Text></Pressable><Pressable style={styles.stopButton} onPress={stop}><Text style={styles.white}>停止</Text></Pressable><Pressable style={[styles.allButton, !shown.length && styles.disabled]} onPress={playAll}><Text style={styles.white}>{playingAll ? '正在播放' : '播放全部'}</Text></Pressable></View>
      <View style={styles.grid}>{shown.map(item => <Pressable key={item.value} style={[styles.item, selected === item.value && styles.itemActive]} onPress={() => select(item)}><Text style={[styles.pinyin, selected === item.value && styles.white]}>{item.value}</Text><Text style={[styles.character, selected === item.value && styles.white]}>{item.name}</Text></Pressable>)}</View>
