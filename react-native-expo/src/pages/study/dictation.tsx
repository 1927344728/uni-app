...
export default function DictationScreen() {
  ...
  return <SafeAreaView style={styles.page}>
    <ScrollView contentContainerStyle={styles.content}>{note ? <View style={styles.note}><Text style={styles.noteText}>{note}</Text></View> : null}
      {!allWords.length ? <View style={styles.empty}><Text style={styles.emptyTitle}>暂无词语</Text><Text style={styles.emptyText}>请打开设置弹层选择词语。</Text></View> : <View style={styles.grid}>{visible.map((item, index) => <Pressable key={`${item.word}-${index}`} onPress={() => { stop(); Speech.speak(item.word, { language: 'zh-CN' }); }} style={[styles.card, status === 'active' && styles.done, status === 'finished' && !completed.has(index) && styles.todo]}><Text style={styles.cardWord}>{item.word}</Text><Text numberOfLines={1} style={styles.cardPinyin}>{item.pinyin}</Text></Pressable>)}</View>}
    </ScrollView>
    <View style={styles.bottom}>{chosen.length ? <Text style={styles.progress}>当前共 <Text style={styles.progressNum}>{chosen.length}</Text> 个{status === 'active' ? <>，已完成 <Text style={styles.progressNum}>{completed.size}</Text> 个，剩余 <Text style={styles.progressNum}>{Math.max(0, chosen.length - completed.size)}</Text> 个</> : null}</Text> : null}<View style={styles.actions}>{status === 'pending' ? <><Pressable style={styles.outline} onPress={() => setSettingsVisible(true)}><Text>设置</Text></Pressable><Pressable style={[styles.primary, !chosen.length && styles.disabled]} onPress={start}><Text style={styles.buttonText}>开始听写</Text></Pressable></> : status === 'active' ? <><Pressable style={styles.danger} onPress={() => { stop(); setStatus('finished'); }}><Text style={styles.dangerText}>结束</Text></Pressable><Pressable style={styles.warning} onPress={togglePause}><Text style={styles.warningText}>{paused ? '继续' : '暂停'}</Text></Pressable><Pressable style={[styles.primary, paused && styles.disabled]} onPress={() => !paused && advance()}><Text style={styles.buttonText}>下一个</Text></Pressable></> : <Pressable style={styles.primary} onPress={restart}><Text style={styles.buttonText}>重新开始</Text></Pressable>}</View></View>
    <Modal visible={settingsVisible} transparent animationType="slide" onRequestClose={() => setSettingsVisible(false)}><View style={styles.mask}><View style={styles.sheet}><Text style={styles.sheetTitle}>设置</Text><ScrollView contentContainerStyle={styles.sheetBody}><Text style={styles.fieldLabel}>间隔时间</Text><TextInput value={String(interval)} onChangeText={value => setIntervalSeconds(clamp(value, 1, 120))} keyboardType="number-pad" style={styles.input} placeholder="请输入词语听写的间隔时间（秒）" /><Text style={styles.fieldLabel}>年级</Text><View style={styles.chips}>{gradeOptions.map(option => <Pressable key={option.value} onPress={() => setGrade(option.value)} style={[styles.chip, grade === option.value && styles.chipActive]}><Text style={grade === option.value && styles.chipActiveText}>{option.label}</Text></Pressable>)}</View><Text style={styles.fieldLabel}>词语数量</Text><TextInput value={String(selected.length)} onEndEditing={event => randomizeCount(event.nativeEvent.text)} keyboardType="number-pad" style={styles.input} /><Text style={styles.fieldLabel}>选择词语</Text><View style={styles.chips}>{available.map(word => <Pressable key={word} onPress={() => setSelected(old => old.includes(word) ? old.filter(item => item !== word) : [...old, word])} style={[styles.chip, selected.includes(word) && styles.chipActive]}><Text style={selected.includes(word) && styles.chipActiveText}>{word}</Text></Pressable>)}</View></ScrollView><View style={styles.sheetActions}><Pressable style={styles.outline} onPress={() => setSettingsVisible(false)}><Text>取消</Text></Pressable><Pressable style={styles.primary} onPress={() => { if (!selected.length) return Alert.alert('提示', '请至少选择一个词语'); restart(); setSettingsVisible(false); }}><Text style={styles.buttonText}>确认</Text></Pressable></View></View></View></Modal>
  </SafeAreaView>;
}
