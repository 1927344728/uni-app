...
export default function VideoScreen() {
  ...
  return <View style={styles.page}><ScrollView contentContainerStyle={styles.content}>
    {!!banners.length && <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={styles.bannerScroll}>{banners.map(item => <Pressable key={String(item.id)} style={styles.banner} onPress={() => openQueue(banners, item)}>{imageUri(item.cover, 700) ? <Image source={{ uri: imageUri(item.cover, 700) }} style={styles.bannerImage} /> : <View style={styles.bannerImage} />}<Text numberOfLines={1} style={styles.bannerTitle}>{plainText((item as Record<string, unknown>).desc)}</Text></Pressable>)}</ScrollView>}
    {!!recommended.length && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recommendations}>{recommended.map(item => <Pressable key={String(item.id)} style={styles.recommendation} onPress={() => openQueue(recommended, item)}>{imageUri(item.cover) ? <Image source={{ uri: imageUri(item.cover) }} style={styles.recImage} /> : <View style={styles.recImage} />}<View style={styles.recText}><Text numberOfLines={1} style={styles.recTitle}>{String(item.title ?? '')}</Text><Text numberOfLines={1} style={styles.recDesc}>{plainText((item as Record<string, unknown>).desc)}</Text></View></Pressable>)}</ScrollView>}
    {tabs.length > 1 && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>{tabs.map(tab => <Pressable key={tab.id ?? 'all'} onPress={() => setType(tab.id)} style={[styles.tab, type === tab.id && styles.tabActive]}><Text style={[styles.tabText, type === tab.id && styles.tabTextActive]}>{tab.name}</Text></Pressable>)}</ScrollView>}
    <View style={styles.grid}>{items.map(item => <Pressable key={String(item.id)} style={styles.card} onPress={() => router.push({ pathname: '/video/play', params: { mode: 'auto', id: String(item.id), type: type ?? '' } })}>{imageUri(item.cover) ? <Image source={{ uri: imageUri(item.cover) }} style={styles.cover} /> : <View style={styles.cover} />}<Text numberOfLines={1} style={styles.title}>{String(item.title ?? '')}</Text><Text numberOfLines={1} style={styles.description}>{plainText((item as Record<string, unknown>).desc)}</Text></Pressable>)}</View>
    {!items.length && <Text style={styles.empty}>~什么都没有哦~</Text>}{!!items.length && <Pressable onPress={() => !last && load(page + 1, true)}><Text style={styles.empty}>{last ? '~没有更多了哦~' : '加载更多'}</Text></Pressable>}
  </ScrollView></View>;
}
