...
# 用法: npm run upload-ssl，或 ./scripts/upload-ssl.sh（Git Bash / WSL / Linux）
# 上传前如需生成 PKCS12，请先执行 npm run convert-p12
...
for dir in "${CERT_DIRS[@]}"; do
  if [[ ! -d "${SSL_DIR}/${dir}" ]]; then
    echo "错误: 找不到目录 ${SSL_DIR}/${dir}" >&2
    exit 1
  fi
done
...
for dir in "${CERT_DIRS[@]}"; do
  ...
  scp "${SCP_OPTS[@]}" "${SSL_DIR}/${dir}" "${REMOTE}:${REMOTE_DIR}/"
done
