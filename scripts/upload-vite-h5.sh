#!/usr/bin/env bash
# 将 uni-vite/dist/build/h5 上传到腾讯云轻量服务器 /opt/www/uni-vue/
# 用法: npm run upload-vite-h5，或 ./scripts/upload-vite-h5.sh（Git Bash / WSL / Linux）

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "${SCRIPT_DIR}/../uni-vite" && pwd)"
BUILD_DIR="${PROJECT_DIR}/dist/build/h5"
ENV_FILE="/d/tencent-cloud-credentials.env"
REMOTE_DIR="/opt/www/uni-vue"
...
if [[ ! -d "$BUILD_DIR" ]]; then
  echo "错误: 找不到构建目录 ${BUILD_DIR}，请先在 uni-vite 下执行 npm run build:h5" >&2
  exit 1
fi
