...
public interface ArticleRepository extends JpaRepository<ArticleEntity, Long> {
  ...
  @Query("SELECT a.id as id, a.type as type, a.subType as subType, a.title as title, a.author as author, a.note as note, a.seq as seq, a.className as className, a.thumb as thumb, a.url as url, a.jumpTo as jumpTo, a.platform as platform FROM ArticleEntity a WHERE (:keyword IS NULL OR :keyword = '' OR a.title LIKE %:keyword%) AND (:type IS NULL OR :type = '' OR CONCAT(',', a.type, ',') LIKE CONCAT('%,', :type, ',%')) AND (:subType IS NULL OR a.subType = :subType) AND (:platform IS NULL OR :platform = '' OR a.platform IS NULL OR a.platform = '' OR CONCAT(',', a.platform, ',') LIKE CONCAT('%,', :platform, ',%')) AND a.isDeleted = false ORDER BY COALESCE(a.seq, 0) DESC, a.id ASC")
