...
public interface BookRepository extends JpaRepository<BookEntity, Long> {
  ...
  @Query("SELECT b.id as id, b.type as type, b.title as title, b.author as author, b.owner as owner, b.description as description, b.score as score, b.seq as seq, b.cover as cover, b.platform as platform FROM BookEntity b WHERE (:keyword IS NULL OR :keyword = '' OR b.title LIKE %:keyword%) AND (:type IS NULL OR :type = '' OR CONCAT(',', b.type, ',') LIKE CONCAT('%,', :type, ',%')) AND (:platform IS NULL OR :platform = '' OR b.platform IS NULL OR b.platform = '' OR CONCAT(',', b.platform, ',') LIKE CONCAT('%,', :platform, ',%')) AND b.isDeleted = false ORDER BY COALESCE(b.seq, 0) DESC, b.updatedTime DESC, b.id ASC")
