...
public interface HomeEntryRepository extends JpaRepository<HomeEntryEntity, Long> {
  @Query("SELECT h FROM HomeEntryEntity h WHERE h.isDeleted = false AND (:platform IS NULL OR :platform = '' OR h.platform IS NULL OR h.platform = '' OR CONCAT(',', h.platform, ',') LIKE CONCAT('%,', :platform, ',%')) ORDER BY h.seq DESC, h.id ASC")
