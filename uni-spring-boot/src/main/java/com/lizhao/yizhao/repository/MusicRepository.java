...
public interface MusicRepository extends JpaRepository<MusicEntity, Long> {
  ...
  @Query("SELECT m FROM MusicEntity m WHERE (:type IS NULL OR :type = '' OR CONCAT(',', m.type, ',') LIKE CONCAT('%,', :type, ',%')) AND (:keyword IS NULL OR :keyword = '' OR m.title LIKE %:keyword%) AND (:platform IS NULL OR :platform = '' OR m.platform IS NULL OR m.platform = '' OR CONCAT(',', m.platform, ',') LIKE CONCAT('%,', :platform, ',%')) AND m.isDeleted = false ORDER BY COALESCE(m.seq, 0) DESC, m.updatedTime DESC, m.id ASC")
