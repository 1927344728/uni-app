...
public interface TaskRepository extends JpaRepository<TaskEntity, Long> {
  ...
  @Query("SELECT t.id as id, t.title as title, t.status as status, t.publisher as publisher, t.targeter as targeter, t.content as content, t.seq as seq, t.progress as progress, t.finished as finished, t.publishTime as publishTime, t.startTime as startTime, t.endTime as endTime, t.platform as platform FROM TaskEntity t WHERE (:keyword IS NULL OR :keyword = '' OR t.title LIKE %:keyword%) AND (:status IS NULL OR t.status = :status) AND (:publisher IS NULL OR :publisher = '' OR t.publisher LIKE %:publisher%) AND (:targeter IS NULL OR :targeter = '' OR t.targeter LIKE %:targeter%) AND (:platform IS NULL OR :platform = '' OR t.platform IS NULL OR t.platform = '' OR CONCAT(',', t.platform, ',') LIKE CONCAT('%,', :platform, ',%')) AND t.isDeleted = false ORDER BY t.seq DESC, t.status ASC")
