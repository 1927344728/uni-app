...
public interface VideoMenuRepository extends JpaRepository<VideoMenuEntity, Long> {
  ...
  @Query("SELECT v FROM VideoMenuEntity v WHERE v.isDeleted = false AND (:platform IS NULL OR :platform = '' OR v.platform IS NULL OR v.platform = '' OR CONCAT(',', v.platform, ',') LIKE CONCAT('%,', :platform, ',%')) ORDER BY v.updatedTime DESC, v.id ASC")
