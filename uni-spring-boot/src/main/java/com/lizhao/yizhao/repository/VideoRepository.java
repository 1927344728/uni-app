...
public interface VideoRepository extends JpaRepository<VideoEntity, Long> {
  ...
  @Query("SELECT v FROM VideoEntity v WHERE (:type IS NULL OR :type = '' OR CONCAT(',', v.type, ',') LIKE CONCAT('%,', :type, ',%')) AND (:keyword IS NULL OR :keyword = '' OR v.title LIKE %:keyword%) AND (:platform IS NULL OR :platform = '' OR v.platform IS NULL OR v.platform = '' OR CONCAT(',', v.platform, ',') LIKE CONCAT('%,', :platform, ',%')) AND v.isDeleted = false ORDER BY v.seq DESC, v.id ASC")
