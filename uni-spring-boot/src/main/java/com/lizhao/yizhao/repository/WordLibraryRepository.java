...
public interface WordLibraryRepository extends JpaRepository<WordLibraryEntity, Integer> {
  ...
  @Query("SELECT w FROM WordLibraryEntity w WHERE w.isDeleted = false AND (:id IS NULL OR w.id = :id) AND (:platform IS NULL OR :platform = '' OR w.platform IS NULL OR w.platform = '' OR CONCAT(',', w.platform, ',') LIKE CONCAT('%,', :platform, ',%')) ORDER BY w.id ASC")
