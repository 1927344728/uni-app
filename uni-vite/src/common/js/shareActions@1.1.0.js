...
const IS_PRODUCT_ENV = _hostname.search(/\.yizhao\.com/) !== -1;
 ...
 *       webPageUrl: 'https://www.yizhao.com/',
...
export function initShareAction(config, options = {}) {
  ...
  detailConfig.map(conf => {
    ...
    if (Object.prototype.toString.apply('') === '[object String]') {
      switch (conf) {
        // APP原生分享信息：小程序
        // http://wy-front.git-page.yizhao.com/lib-app-webview-bridge1/#/api?id=通用-actionsheet-接口
        case 'miniProgramData': {
          appShareConfig.push({
            action: conf,
            webPageUrl: 'https://www.yizhao.com/', // 兼容低版本的网页链接
...
/**
 * APP 内 H5 点击分享
 * http://wy-front.git-page.yizhao.com/lib-app-webview-bridge1/#/api?id=直接显示-actionsheet
